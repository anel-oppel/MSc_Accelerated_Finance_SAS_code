...
call sort(x,ncol(x));
*print x;
create dissn.relia1 from x[colname=names];
append from x;
quit;
data dissn.fin_relia;
set dissn.relia1(drop=obs);
obs=_n_;
...
data dissn.fin_relia;
set dissn.fin_relia(drop= IT r markCapp M);
run;
quit;
...
proc reliability data=dissn.fin_relia;
...
make 'modprmest' out=dissn.fin_relia_graphMEst;
make 'parmest' out=dissn.fin_relia_graphest;
rplot (t1 t2)= Mc/
      pplot
      fit = model
	  relation=pow
